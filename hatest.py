...
♧ GITHUB     :  https://github.com/Rk-Black/ •                 ♧
♧ WhatsApp :  +17867244875                                             ♧
♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧♧
			{m}•{k}•{h}•{sir} Author : MD Riyadul Momin{x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'>> 1. RIYAD {h}OK{x} Anda ')
	print(f'>> 2. RIYAD {k}CP{x} Anda ')
	print('>> 3. Exit	')
	kz = input(f'\n>> Pilih : ')
	if kz in ['2']:
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			print('>> You Don't Have The File OK ')
			time.sleep(2)
			back()
		else:
			cih = 0
